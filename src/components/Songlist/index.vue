<template>
	<div class="songlist">
		<div
			class="songlist_header"
			v-if="!isSmallScreen"
		>
			<div class="songlist_index"></div>
			<div class="songlist_title">标题</div>
			<div
				class="songlist_singer"
				v-if="!isSmallScreen"
			>
				歌手
			</div>
			<div
				class="songlist_duration"
				v-if="!isSmallScreen"
			>
				时间
			</div>
			<div
				class="songlist_option"
				v-if="isSmallScreen"
			></div>
		</div>
		<div class="songlist_body">
			<div
				v-if="!loading && !loadingError && songs.length === 0"
				class="songlist_empty"
			>
				这里什么都没有
			</div>
			<div
				...
				:class="`songlist_item ${index % 2 === 0 ? 'stripe' : ''} ${
					activeId === song.id && activeCid === song.cid ? 'active' : ''
				} ${song.st === -200 ? 'disable' : ''}`"
				:draggable="canDrag"
			...
			>
				...
				<div class="songlist_title">
					<div class="songlist_title-title">
						<span
							class="songlist_title-text"
							:title="song.title"
							>{{ song.title }}</span
						>
						<span
							v-if="song.fee === 1"
							class="el-icon songlist_title-icon"
							><IconCusVip
						/></span>
					</div>
					<div
						v-if="isSmallScreen"
						class="songlist_title-singer"
						:title="song.singers.map((s: Singer) => s.name).join(' / ')"
					>
				...
				<div
					...
					v-if="isSmallScreen"
				>
					<el-icon @click.stop="(e: MouseEvent) => showPopbox(e, song)"
						><IconAntDesignMoreOutlined
					/></el-icon>
				</div>
			</div>
		</div>
...
<script lang="ts" setup>
	...
	async function playSong(event: MouseEvent, song: MusicInfo) {
		if (
			audioInfo.value.id !== song.id ||
			(audioInfo.value.type === AudioInfoType.bili && audioInfo.value.cid !== song.cid)
		) {
	...
	function dragover(e: DragEvent) {
		...
		let el = e
			.composedPath()
			.find((el) => Array.from((el as HTMLElement).classList).includes('songlist_item')) as HTMLElement;

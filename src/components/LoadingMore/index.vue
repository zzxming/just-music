<template>
	<div class="load_wrapper">
		...
		></slot>
		<div
			:class="`load_more ${isStatic ? '' : 'load_gap'}`"
			ref="loadMore"
		>
			<div
				v-if="!haveMore && !isStatic"
				class="load_txt load_nomore"
			>
				已经没有更多了
			</div>
			<template v-if="haveMore">
				<div v-show="!loadingError">
					<span
						v-show="loading"
						:class="`load_txt ${isStatic ? 'load_gap' : ''}`"
						>加载中...</span
					>
					<span
						v-show="!loading && !isStatic"
						class="load_txt load_btn"
						@click="loadFunc"
						>加载更多</span
					>
				</div>
				<LoadingErrorTip
					:isError="!loading && loadingError"
...
<script lang="ts" setup>
	...
	function observerLoad() {
		...
		let loadIO = new IntersectionObserver(
			async (entries) => {
